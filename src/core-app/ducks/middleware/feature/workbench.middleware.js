...
const middleware =
  ...
  async (action) => {
    ...
    if (action.type === 'PING')
      console.log(`%cPING recieved by workbench.middleware`, colors.light.purple);
    ...
    switch (action.type) {
      ...
      case DND_DRAG_END: {
        ...
        if (typeof destination === 'undefined' || typeof source === 'undefined') {
          break;
        }
      ...
      case MOVE_TREE: {
        ...
        const srcPalette = treeState[source.droppableId].type === NODE_TYPES.PALETTE;
        const dstPalette =
      ...
      case `${WORKBENCH} ${POLLING_RESOLVED}`: {
        ...
        try {
          ...
          if (id !== projectId) {
            throw new InvalidStateError(`Mismatch project: graphql and middleware`);
          }
          ...
          const tree = Tree.fromObsEtl(
            {
              qualities: subject.qualities.map(iniEtlUnitQual(subject.subjectType)),
              measurements: measurements.map(iniEtlUnitMea({ etlFields, etlUnits })),
            },
